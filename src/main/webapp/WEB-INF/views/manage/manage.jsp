				...
				.click(
						function() {
							...
							$.each(managers, function(index, value) {
								...
								var name = value.name;
								var btnAssignHtml = $("<tr><td>"+staffId+"</td><td>"+name+"</td><td><button class='btn btn-default .btn-assign-mng'><i class='glyphicon glyphicon-thumbs-up'></i></button></td></tr>");
								$('#mnglist-tbody').append(btnAssignHtml.click(function(){$('#hostManagerId').val(staffId);$('#hostManagerName-show').val(name);$('#opt-staffid').val("Staff (assign to "+name+")");$('#modal-assign-mng').modal('hide');}));
							});
