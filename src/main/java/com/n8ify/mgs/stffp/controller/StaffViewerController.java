...
public class StaffViewerController {
	...
	@RequestMapping(value = "/viewAllAssignedStaffs", method = RequestMethod.GET)
	public String viewAllAssignedStaffs(Model model,
			@RequestParam(value = "json", required = true, defaultValue = "false") boolean json) {
		if (json) {
			gsonb.serializeNulls();
			model.addAttribute("results", gsonb.create().toJson(staffManager.getTotalAssignedStaffs()));
			return "result/result";
		}
		model.addAttribute("staffs", staffManager.getEntireStaffs());
		return "list";
